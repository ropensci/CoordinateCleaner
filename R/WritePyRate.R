WritePyRate <- function(x, taxon = "accepted_name", min.age = "min_ma", max.age = "max_ma", 
    ...
    cat("#!/usr/bin/env python", "from numpy import * ", "", file = outfile, 
        sep = "\n")
    ...
    for (j in 1:replicates) {
        ...
        if (isTRUE(random)) {
            dat$new_age <- round(stats::runif(length(dat[, 1]), min = apply(dat[, 
                3:4], FUN = min, 1), max = apply(dat[, 3:4], FUN = max, 1)), 
                digits = 6)
        } else {
        ...
        dat3 <- matrix(data = NA, nrow = length(times), ncol = max(sapply(times, 
            length)))
        rownames(dat3) <- taxa
        ...
        for (n in seq_len(length(taxa) - 1)) {
            ...
            cat(noquote(sprintf("array([%s]),", rec)), file = outfile, append = TRUE, 
                sep = "\n")
        }
    ...
    cat(noquote(sprintf("d=[%s]", data_sets)), noquote(sprintf("names=[%s]", 
        names)), "def get_data(i): return d[i]", "def get_out_name(i): return  names[i]", 
