CleanCoordinatesDS <- function(x, lon = "decimallongitude", lat = "decimallatitude", 
    ds = "dataset", ddmm = TRUE, periodicity = TRUE, ddmm.pvalue = 0.025, ddmm.diff = 0.2, 
    periodicity.T1 = 7, periodicity.reg.thresh = 2, periodicity.dist.min = 0.1, 
    periodicity.dist.max = 2, periodicity.min.size = 4, periodicity.target = "both", 
    ...
    if (ddmm) {
        out.t1 <- dc_ddmm(x = dat, lon = lon, lat = lat, ds = ds, pvalue = ddmm.pvalue, 
    ...
    if (periodicity) {
        out.t2 <- dc_round(x, lon = lon, lat = lat, ds = ds, T1 = periodicity.T1, 
