...
CleanCoordinatesFOS <- function(x, lon = "lng", lat = "lat", min.age = "min_ma", 
    max.age = "max_ma", taxon = "accepted_name", countries = "cc", centroids = TRUE, 
    countrycheck = TRUE, equal = TRUE, GBIF = TRUE, institutions = TRUE, temp.range.outliers = TRUE, 
    spatio.temp.outliers = TRUE, temp.ages.equal = TRUE, zeros = TRUE, centroids.rad = 0.05, 
    centroids.detail = "both", inst.rad = 0.001, outliers.method = "quantile", 
    outliers.threshold = 5, outliers.size = 7, outliers.replicates = 5, zeros.rad = 0.5, 
    centroids.ref, country.ref, inst.ref, value = "spatialvalid", verbose = TRUE, 
    report = FALSE) {
    ...
    if (spatio.temp.outliers) {
        ...
        if (taxon != "") {
            ...
            otl.flag <- tc_outl(x = x, lon = lon, lat = lat, min.age = min.age, 
    ...
    if (temp.range.outliers) {
        ...
        test <- x
        ran.otl <- tc_range(x = test, taxon = "", min.age = min.age, max.age = max.age, 
        ...
        if (taxon != "") {
            ...
            ran.otl.flag <- tc_range(ran.test, taxon = taxon, min.age = min.age, 
    ...
    # Temporal, Equal ages
    if (temp.ages.equal) {
        age.equ <- tc_equal(x, min.age = min.age, max.age = max.age, value = "flags", 
            verbose = verbose)
    } else {
