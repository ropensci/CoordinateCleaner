...
CleanCoordinates <- function(x, lon = "decimallongitude", lat = "decimallatitude", 
    species = "species", countries = NULL, capitals = TRUE, centroids = TRUE, 
    countrycheck = FALSE, duplicates = FALSE, equal = TRUE, GBIF = TRUE, institutions = TRUE, 
    outliers = FALSE, seas = TRUE, urban = FALSE, zeros = TRUE, capitals.rad = 0.05, 
    centroids.rad = 0.01, centroids.detail = "both", inst.rad = 0.001, outliers.method = "quantile", 
    outliers.mtp = 3, outliers.td = 1000, outliers.size = 7, zeros.rad = 0.5, 
    capitals.ref, centroids.ref, country.ref, inst.ref, seas.ref, urban.ref, 
    ...
    ## Equal coordinates
    if (equal) {
        equ <- cc_equ(x, lon = lon, lat = lat, verbose = verbose, value = "flags", 
            test = "absolute")
    ...
    ## Zero coordinates
    if (zeros) {
        zer <- cc_zero(x, lon = lon, lat = lat, buffer = zeros.rad, verbose = verbose, 
            value = "flags")
    ...
    ## Capitals
    if (capitals) {
        cap <- cc_cap(x, lon = lon, lat = lat, buffer = capitals.rad, ref = capitals.ref, 
    ...
    ## Centroids
    if (centroids) {
        cen <- cc_cen(x, lon = lon, lat = lat, buffer = centroids.rad, test = centroids.detail, 
    ...
    # Seas
    if (seas) {
        sea <- cc_sea(x, lon = lon, lat = lat, ref = seas.ref, verbose = verbose, 
            value = "flags")
    ...
    # Urban Coordinates
    if (urban) {
        urb <- cc_urb(x, lon = lon, lat = lat, ref = urban.ref, verbose = verbose, 
            value = "flags")
    } else {
        urb <- rep(NA, dim(x)[1])
    }
    ...
    # Country check
    if (countrycheck) {
        con <- cc_coun(x, lon = lon, lat = lat, iso3 = countries, ref = country.ref, 
    ...
    if (institutions) {
        inst <- cc_inst(x, lon = lon, lat = lat, ref = inst.ref, buffer = inst.rad, 
            verbose = verbose, value = "flags")
    } else {
        inst <- rep(NA, dim(x)[1])
    }
    ...
    if (value == "spatialvalid") {
        inp <- data.frame(species = x[, species], decimallongitude = x[, lon], 
            decimallatitude = x[, lat])
        out <- data.frame(inp, validity = val, equal = equ, zeros = zer, capitals = cap, 
