...
So far so good, we have significantly refined the data  for our needs. In section 6 we will have a look at the metadata for further refinement, but before, note that there are two different ways to run \emph{CoordinateCleaner}. You can  connect all functions directly in a row using the magrittr pipe (%>%) operator.
