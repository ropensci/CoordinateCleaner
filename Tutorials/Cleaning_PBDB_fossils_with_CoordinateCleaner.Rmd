...
# Background
The public availability of fossils for large-scale analyses is rapidly increasing, mainly due to increased databasing efforts and data aggregators such as the paleobiology database (www.paleobiodb.org) and Neotoma (www.neotomadb.org), among others. However, data quality is an issue, in particular for old collections or collections with uncertain taxonomy and/or bad preservation. Problems in general biological collection databases (See supplementary material S2) are also relevant for fossils. In addition, fossils may be dated wrongly or with low precision. This tutorial presents a pipeline to clean fossil data from a paleobiology database before using it in biogeographic or evolutionary analyses. We focus on identifying imprecisely geo-referenced and/or dated records by combining automated cleaning using \emph{CoordinateCleaner} with cleaning based on metadata. The proposed steps are by no means exhaustive, and keep in mind that what is "good data" depends entirely on your downstream analyses! We wrote this tutorial (and the whole \emph{CoordinateCleaner} package) to help you to identify potential problems more quickly and  reproducibly to improve data quality in large datasets. In this tutorial, we will mostly remove flagged records from the dataset. However, we recommend that you double check them individually.
...
# Load test dataset
For this tutorial, we will use a dataset of vascular plant fossils from the last 65 million years, downloaded from a paleobiology database using the [paleobioDB package](https://github.com/ropensci/paleobioDB). For the tutorial, we'll limit the data to maximum 10000 records to keep the download time reasonable. If you obtained your data from the web mask of the paleobiology database, or use an entirely different database, you will have to adapt the column names in the script.
...
For the purpose of this tutorial, we will always exclude flagged records. If you want to further explore them, use the `value = "flags"` argument, valid for all functions. In that case the output value will be a vector of logical values with the same length as dat, where TRUE = valid record, FALSE = flagged record. It is generally advisable to check flagged records whenever possible, to avoid data-loss and false flags.
...
Some individual records are dated with a precision of more than 60 milion years! \emph{CoordinateCleaner} offers three ways to flag records based on their age range: (1) based on absolute age, e.g. age range > 35 million years; (2) based on age range outlier detection in the entire dataset (e.g. if few records are much less precisely dated than the rest of all records); and (3) based on age range outlier detection on taxon level (e.g. all \emph{Quercus} records that are much less precisely dated than the other \emph{Quercus} records). The second and third approaches can be combined and offer more flexibility over the absolute age limit, but need some consideration on the desired sensitivity. Here, we will run all three variants as an illustration. If you use your own data, you should decide which is most suitable based on your downstream analyses. In the case of (2) and (3), you can tweak the test sensitivity using the `mltpl` argument.
...
Finally, we will test for outliers in space-time--that is, records that are very distant in either space or time from all other records in the dataset or per taxon. The test is again based on quantile outlier detection and can be modified using  various arguments. Here it is important to carefully consider the desired test sensitivity. See `?tc_outl` for help.
...
All tests combined have removed about `r nrow(dat) - nrow(cl)` records (`r round((1 - nrow(cl) / nrow(dat)) *100, 1)`%). If you want to identify all flagged records, to double check or correct them, take a look at the `CleanCoordinatesFOS` wrapper function below. 

So far so good--we have significantly refined the data  for our needs. In section 6, we will have a look at the metadata for further refinement, but before that, note that there are two different ways to run \emph{CoordinateCleaner}. You can  connect all functions directly in a row using the magrittr pipe (%>%) operator.
...
# Improving data quality using metadata
Usually, at least some type of metadata are provided with fossil occurrences, as is the case in the paleobiology database. We'll now explore these and see if we can identify further problems.  
...
(1) all taxa in our dataset are plants and (2) they are at least identified to genus level.
...
## Time 
We have checked for potentially problematic records in time and space above, but it is advisable to check again.
...
## Additional metadata
If you download data using the paleobiology database web portal, there are some additional fields you can check to generally gauge data precision, namely basis and precision of the geo-reference, the year of publications (old publications are more likely to be imprecise), preservation quality (bad preservation increases taxonomic uncertainty), plant organ (also taxonomic uncertainty), and collection type. You may need to adapt the column names, and not all columns will be relevant for all taxa (e.g. "plant organ").
...
If you want to use fossil data to estimate speciation and extinction rates, to explore their correlation with environmental factors, or to do biogeography, PyRate (https://github.com/dsilvestro/PyRate) may be useful for you. You can use the `WritePyrateOut` function of \emph{CoordinateCleaner} to write ready-to-use PyRate input files from your cleaned dataset to disk. To to so, you additionally need to provide an assessment of which species are extinct and which are extant in the `status` argument of `WritePyRate`. If you want to specify a path for saving the file, use the `path` argument instead of `fname`.
