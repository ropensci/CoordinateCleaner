---
title: "Quick start - problemaitc records"
author: "Alex Zizka"
date: "24 July 2018"
...
```{r}
#run all tests
dat <- clean_coordinates(exmpl, tests = c("capitals", "centroids", "countries", "equal", "gbif", "institutions", "outliers", "seas", "urban", "zeros"))
...
|gbif|radius around GBIF headquaters|data entry errors, falsely geo-referenced|on|
...
head(capitals) #a data.frame with four columns: ISO3, capital, longitude, latitude
...
|---|---|---|---|
|capitals|capitals|`data.frame`|capitals.ref|
|centroids|centroids|`data.frame`|centroids.ref|
|countrycheck|countryborders|`SpatialPolygonsDataFrame`|country.ref|
|institutions|institutions|`data.frame`|inst.ref|
|seas|landmass|`SpatialPolygonsDataFrame`|seas.ref|
|urban|none|`SpatialPolygonsDataFrame`|urban.ref|
...
clean <- inp[, flags$sumary]

#exclude records flagged by the centroids test
clean <- inp[, flags$centroids]
```
